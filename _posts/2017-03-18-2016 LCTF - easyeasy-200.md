...
![1.png](resources/D7929F9A772913A9A5C7BC4323315E20.png =1155x521)

调用`Format类`的函数进行处理，这里写了这么多个，肯定在后面会用到，不然出题人吃饱撑着写这么多

![2.png](resources/B9BACA287C5607274E12EBC3646CC73F.png =668x298)

`check()`函数里显示检测了模拟器，所以直接运行在真机上，然后调用native函数进行校验

![3.png](resources/B671016B48EC4E567CC9873AB788B104.png =916x416)

IDA载入so，先看`checkEmulator()`函数，这个函数并没有做其它操作，只是根据传入的值返回

![4.png](resources/17E19CB1737E8290A924B2C36AEB298E.png =1920x1050)

同时，我们看到还有一个`checkPasswd()`函数

我们进行简单的分析，可以看出来中间用到了某种加密操作

![5.png](resources/BD021A61E412E73104873CFDF93410CB.png =1313x679)
...
![6.png](resources/1C5C70F2313FAD8DE98F3FBEAF94B2D8.png =1919x1050)
...
![7.png](resources/33D397BEE09213CCCC91EBD701047CD3.png =1540x761)

粗略一看，这里面的东西还是很多的，先是做初始化，猜测这里是要获取dex文件

![8.png](resources/69EA190E3646DA05E4BDF00AB796B149.png =1035x595)

通过`/proc/self/maps`找dex文件

![9.png](resources/620241D4E9DC523CE68E049B3668B82F.png =891x433)

寻找dex的Magic Number

![10.png](resources/EB8D6B6D08A540A574F04A6D7E5C1D5C.png =784x166)
